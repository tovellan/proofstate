...
```sh
uv tool install git+https://github.com/tovellan/proofstate@v0.3.6
proofstate --version
```
...
No package is published to PyPI in version 0.3.6.
...
## Verify portable conformance fixtures

The wheel includes a digest-pinned fixture corpus for independent `v1alpha1`
implementations. Verify the installed copy and print machine-readable outcomes:
...
```sh
proofstate schema scorecard
proofstate schema attestation
```
...
step before ProofState. Version 0.3.6 does not fetch remote evidence, execute
...
clean wheel and source-archive installations, the executable example,
tracked-text checks, and a full-history secret scan when `gitleaks` is installed.

## Status

ProofState 0.3.6 supports local Git worktrees and SHA-1 or SHA-256 Git object
